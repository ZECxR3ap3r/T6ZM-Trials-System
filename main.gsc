...
onplayerspawned()
{
	...
	for ( ;; )
	{
		...
		if(!isdefined(self.initial_spawn))
		{
			...
			self.ReaperTrialsCurrentMagic = 99;
			self init_trial_hud();
		}
	}
}
	...
	while(1){
		...
		if(player == players[num]){
			...
			players[num].ReaperTrialsCurrentMagic = 0;
      		...
      		wait 0.5;
			RewardModel = Spawn( "script_model", self.origin + (0,0,40));
